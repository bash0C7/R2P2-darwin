...
- **#4 net-TLS** route1 darwin 対応は実装済（mrbgem.rake の posix||darwin 分岐 + net.h guard）。**full build はブロック**: dep `picoruby-pack` が mruby VM 版欠落（`src/mruby/pack.c` 無し）で `PICORB_VM_MRUBY` 時 fatal。darwin port と無関係な upstream gap。完遂には pack の mruby 実装が前提（別タスク）。
