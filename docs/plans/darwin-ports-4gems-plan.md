...
- **#4 net-TLS** route1 完了・検証済。fork `net/darwin-port`（mrbgem.rake の posix||darwin 分岐 + net.h guard 2 箇所）。build_config `...-net-sim.rb`。dep の picoruby-pack は mruby VM 版欠落（`src/mruby/pack.c` 無し、単体でも fatal）のため、**build_config 側で** picoruby-pack 依存を剥がし標準 `mruby-pack` に差し替え（net の C・依存とも picoruby-pack の C API 不使用を確認済み）。libmruby.a に `_DNS_resolve`/`_TCPClient_send`/`_mbedtls_hardware_poll`/`_rng_random_byte_impl`=T、`_SecRandomCopyBytes`=U を確認。route2(Network.framework) は ATS 必須化まで保留。
